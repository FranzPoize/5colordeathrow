Crafty.c("MoveTo", 
{
	...
	init: function ()
	{
		...
		this.bind('MouseUp',this._onmouseup);
		this.bind('MouseMove',this._onmousemove);
	...
	_update: function ( frame )
	{
		...
		if(this.x < 0 || this.x + this.w > Crafty('PlayArea').w) {
			this._vVelocity.x = -this._vVelocity.x;
			if (this.x < 0)
				this.x = 0;
			else
				this.x = Crafty('PlayArea').w - this.w
			Crafty.audio.play('player_bounce',1);
		}
		if(this.y < 0 || this.y + this.h > Crafty('PlayArea').h) {
			this._vVelocity.y = -this._vVelocity.y;
			if (this.y < 0)
				this.y = 0;
			else
				this.y = Crafty('PlayArea').h - this.h
			Crafty.audio.play('player_bounce',1);
		}
	},
